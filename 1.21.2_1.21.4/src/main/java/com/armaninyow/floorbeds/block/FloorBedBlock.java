...
public class FloorBedBlock extends BedBlock {
	...
	public float calcBlockBreakingDelta(BlockState state, PlayerEntity player, BlockView world, BlockPos pos) {
		...
		if (isShears) {
			...
			if (efficiencyOpt.isPresent()) {
				int level = EnchantmentHelper.getLevel(efficiencyOpt.get(), tool);
				if (level > 0) speedMultiplier += (level * level) + 1;
			}
		} else {
		...
		var aquaOpt = enchantmentRegistry.getOptional(Enchantments.AQUA_AFFINITY);
		if (aquaOpt.isPresent()) {
			hasAquaAffinity = EnchantmentHelper.getEquipmentLevel(aquaOpt.get(), player) > 0;
		}
		if (player.isSubmergedInWater() && !hasAquaAffinity) speedMultiplier /= 5.0f;
	...
	protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
		...
		if (state.get(PART) != BedPart.HEAD) {
			...
			if (!(state.getBlock() instanceof FloorBedBlock)) return ActionResult.CONSUME;
		}
		...
		try {
			player.trySleep(pos).ifLeft(reason -> {
				if (reason != null) {
					// Map enum constants to vanilla translation keys.
					// NOT_POSSIBLE_HERE = Nether/End — show no message,
					// matching vanilla and 1.21.5_1.21.11 behaviour where message() returns null.
					String key = switch (reason) {
						case NOT_POSSIBLE_HERE -> null;
						case NOT_POSSIBLE_NOW  -> "block.minecraft.bed.no_sleep";
						case TOO_FAR_AWAY      -> "block.minecraft.bed.too_far_away";
						case OBSTRUCTED        -> "block.minecraft.bed.obstructed";
						case NOT_SAFE          -> "block.minecraft.bed.not_safe";
						case OTHER_PROBLEM     -> "block.minecraft.bed.no_sleep";
					};
					if (key != null) {
